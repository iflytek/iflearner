...
parser.add_argument(
    "--data",
    metavar="DIR",
    default="/data1/shengxu8/projects/iflytek/data/tiny-imagenet-200-iid-0",
...
class ImageNet(pytorch_trainer.PyTorchTrainer):
    def __init__(
        self, train_loader, val_loader, model, criterion, optimizer, args, scheduler
    ) -> None:
        ...
        self._scaffold = bool(args.scaffold)
        self._best_acc1 = 0

    def config(self) -> dict():
        print(self._scheduler.get_lr(), self._scheduler.get_last_lr())
        print(
            f"learning_rate: {self._scheduler.get_lr()}, batch_size: {len(self._train_loader)}"
        )
        return {
            "learning_rate": self._scheduler.get_lr()[0],
            "batch_size": len(self._train_loader),
        }
    ...
    def evaluate_traindata(self):
        ...
        progress = ProgressMeter(
            len(self._train_loader), [batch_time, losses, top1, top5], prefix="Test: "
        )
    ...
    def fit(self, epoch):
        ...
        for _ in range(1):
            for i, (images, target) in enumerate(self._train_loader):
                ...
                if i % self._args.print_freq == 0:
                    progress.display(i)
        self._new_weights = self._model.state_dict()
    ...
    def evaluate(self, epoch):
        (
            ...
            top1,
            top5,
        ) = validate(self._val_loader, self._model, self._criterion, self._args)

        self._scheduler.step()

        if float(top1) > self._best_acc1:
            save_checkpoint(
                {
                    "epoch": epoch + 1,
                    "arch": self._args.arch,
                    "state_dict": self._model.state_dict(),
                    "best_acc1": self._best_acc1,
                    "optimizer": self._optimizer.state_dict(),
                    "scheduler": self._scheduler.state_dict(),
                },
                True,
                self._args.name + "checkpoint.pth.tar",
            )
            self._best_acc1 = float(top1)

        return {"loss": loss, "top1": float(top1), "top5": float(top5)}
    ...
    def get(self, param_type=""):
        ...
        for name, p in self._model.named_parameters():
            if p.requires_grad:
                parameters[name.replace("module.", "")] = p.cpu().detach().numpy()
    ...
    def set(
        self,
        parameters,
        param_type=pytorch_trainer.PyTorchTrainer.ParameterType.ParameterModel,
    ) -> None:
...
def main_worker(gpu, ngpus_per_node, args):
    ...
    if args.dp:
        privacy_engine = PrivacyEngine(
            model,
            sample_rate=args.batch_size / len(train_dataset) * 2,
            epochs=args.epochs,
    ...
    imagenet = ImageNet(
        train_loader, val_loader, model, criterion, optimizer, args, scheduler
    )
    controller = train_client.Controller(args, imagenet)
    controller.run()
...
def train(train_loader, model, criterion, optimizer, epoch, args, inst):
    batch_time = AverageMeter("Time", ":6.3f")
    data_time = AverageMeter("Data", ":6.3f")
    losses = AverageMeter("Loss", ":.4e")
    top1 = AverageMeter("Acc@1", ":6.2f")
    ...
    for i, (images, target) in enumerate(train_loader):
        ...
        g = yield inst.get(inst.ParameterType.ParameterGradient)
        inst.set(
            homo_pb2.AggregateResult(parameters=g.parameters),
            inst.ParameterType.ParameterGradient,
        )
