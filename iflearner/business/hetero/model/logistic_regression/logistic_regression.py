...
class ClientA(Client):
    ...
    def task_1(self, client_B_name):
        ...
        ), "Error: 'public_key' from C in step 1 not successfully received."
    ...
    def task_2(self, client_C_name):
        dt = self.data
        assert "encrypted_u_b" in dt.keys(
        ), "Error: 'encrypted_u_b' from B in step 1 not successfully received."
    ...
    def task_3(self):
        dt = self.data
        assert "masked_dJ_a" in dt.keys(
        ), "Error: 'masked_dJ_a' from C in step 2 not successfully received."
...
class ClientB(Client):
    ...
    def task_2(self, client_C_name):
        try:
            ...
            ), "Error: 'encrypt_u_a' from A in step 1 not successfully received."
        ...
        try:
            assert "encrypted_z_a_square" in dt.keys(
            ), "Error: 'encrypted_z_a_square' from A in step 1 not successfully received."
            encrypted_z = 4*encrypted_u_a + dt['z_b']
            encrypted_loss = np.sum((0.5-self.y)*encrypted_z + 0.125 *
                                    dt["encrypted_z_a_square"] + 0.125*dt["z_b"] * (encrypted_z+4*encrypted_u_a))
        except Exception as e:
            print("B step 2 exception: %s" % e)
    ...
    def task_3(self):
        try:
            dt = self.data
            assert "masked_dJ_b" in dt.keys(
            ), "Error: 'masked_dJ_b' from C in step 2 not successfully received."
...
class ClientC(Client):
    ...
    def task_2(self, client_A_name, client_B_name):
        try:
            dt = self.data
            assert "encrypted_masked_dJ_a" in dt.keys() and "encrypted_masked_dJ_b" in dt.keys(
        ...
        try:
            ...
            ), "Error: 'encrypted_loss' from B in step 2 not successfully received."
            encrypted_loss = dt['encrypted_loss']
            loss = self.private_key.decrypt(
                encrypted_loss) / self.A_data_shape[0] + math.log(2)
            print("******loss: ", loss, "******")
...
def vertical_logistic_regression(X, y, X_test, y_test, config):
    ...
    # 获取数据
    XA, XB, XA_test, XB_test = vertically_partition_data(
        X, X_test, config['A_idx'], config['B_idx'])
